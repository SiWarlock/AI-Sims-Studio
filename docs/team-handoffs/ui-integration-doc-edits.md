...
| A7 | §4 | **`PUT /settings` = FULL-REPLACE (user-pinned this round, reversing the implicit partial-PUT assumption).** The Phase-2 `/settings` handler MUST replace the full resource (an omitted optional field resets to its default) and ship with a **contract acceptance test** enforcing full-replace (lead-owned; the core track inherits this at integration). UI corrected to **read-modify-write** the full `SettingsResponse` (brief `ui-009` — fixes the latent data-loss in shipped `persistModsPath` + `persistTelemetryEnabled`; mock flipped PATCH-merge→full-replace). **OPEN QUESTION — DEFERRED to Phase 2** (lead decision, post-round-4): under full-replace, `UpdateSettingsRequest`'s optional fields are a footgun (omitting one resets it). The tradeoff is decided when contracts/core builds the §4 settings model + handler: **required fields** (safe under full-replace but rigid — adding a new setting becomes a breaking change) vs **optional + server-side defaults** (evolvable, the user's usual lean, but relies on the no-reset / read-modify-write discipline the UI now has). Not blocking now — the migration + this pin suffice; decided/escalated when the Phase-2 model lands. | finding(ui)→user §4 call | QUEUED |
...
| T15 | 2 / pipeline | **Phase-2 supervisor boot-selection** — swap `InMemorySecretsAccessor` → `KeychainSecretsAccessor` at the sidecar supervisor/app-context boot, supplying the real configured provider-id list + the real `keyring` backend (the production entry point for the 7.2b-2 lib). Also: **`services/pipeline/LESSONS.md` Convention** (apply at integration, reconciling with the core track) — "the sidecar keychain READ is a new `SecretsAccessor` Protocol impl (keyring-backed, injected seam, names-only repr, **fail-safe** `active_values()`, typed-redacted unavailable error) at the ui-owned `(service='AISimsCreator', account=providerId)` contract; zero edits to `obs/secrets.py` — the redactor stays drop-in." | 7.2b-2 | QUEUED |
