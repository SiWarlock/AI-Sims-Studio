...
3. Read the area's cross-doc invariants table in `<area>/CLAUDE.md` — the pin matrix.
...
<!-- ▼ EXAMPLE BLOCK [id=safety-invariant-cross-checks]: project safety-invariant cross-checks — replace wholesale with the project's actual key safety rules + the specific cross-checks for each. ▼ -->
   ...
   - **Exportability gate** — an item is exportable only if `included ∧ has a selected AssetVariant ∧ no blocking validation`. Trace any code that sets, clears, or reads an export-eligible flag or builds an export job list; confirm all three conjuncts are checked together and none is bypassed (e.g. an item queued for export without a selected AssetVariant, or with a blocking validation result ignored). Report PASS or FINDING with file:line + cited spec anchor.
   - **FunctionalOverlay identity** — a FunctionalOverlay is the SAME ItemSpec identity, never a duplicate item. Grep for overlay creation / clone / copy paths; confirm the overlay reuses the existing ItemSpec id and does not mint a new item id or fork the item record. Report PASS or FINDING.
   - **Sidecar sole-writer** — the sidecar is the SOLE writer of Postgres + the canonical artifact tree; workers write only to sidecar-provided scratch dirs and return paths. If the slice is in a worker area (`workers/blender`, `workers/export`), confirm it never opens a DB connection or writes outside its provided scratch dir; if in the sidecar, confirm worker-returned paths are validated before any canonical move. Report PASS or FINDING.
   - **Atomic export safety** — export = temp-write → fsync → DBPF round-trip validate → atomic rename; donors opened read-only; NEVER build/write a half-package in the live Sims Mods folder. Trace any file write in the export path; confirm the temp→fsync→validate→rename ordering holds, donor handles are read-only, and no write targets the live Mods folder directly. Report PASS or FINDING.
   - **Secret handling** — secrets (cloud/LLM keys) live ONLY in the OS keychain; never in DB/logs/traces; redaction at every egress; tracing is fail-open (never blocks a run). Grep the diff for any path that persists, logs, or traces a key/credential; confirm keychain is the only store, redaction wraps every egress, and tracing failures never raise into the run path. Report PASS or FINDING.
   - **Deterministic validation before state writes + gate ordering** — cloud/agent/MCP outputs pass deterministic validation before any state write; the 5 approval gates are ordered (plan→concept→mesh→overlay→export). Confirm any state mutation downstream of a cloud/agent/MCP call is preceded by deterministic validation, and that gate transitions cannot skip or reorder the plan→concept→mesh→overlay→export sequence. Report PASS or FINDING.
