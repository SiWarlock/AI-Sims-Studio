...
plus a parseErrorCode tolerance helper (generated/helpers.ts).
...
def check() -> bool:
    ...
    helpers). True = clean. Needs Node (the TS half); ``schema_matches`` is the pure-Python half."""
    with tempfile.TemporaryDirectory() as tmp:
        generate(Path(tmp))
        for name in ("contracts.schema.json", "contracts.ts", "helpers.ts"):
