...
**Area-resolved dependency audit.** The dependency-audit row runs a per-area audit command (`$AUDIT_CMD`) per touched area; resolve it from each touched area's basename:

```bash
case "<touched-area-basename>" in
  desktop)   AUDIT_CMD="pnpm audit --prod" ;;
  pipeline)  AUDIT_CMD="uv run pip-audit" ;;
  export)    AUDIT_CMD="pnpm audit --prod" ;;
  blender)   AUDIT_CMD="uv run pip-audit" ;;
  contracts) AUDIT_CMD="uv run pip-audit" ;;
  evals)     AUDIT_CMD="uv run pip-audit" ;;
esac
```
