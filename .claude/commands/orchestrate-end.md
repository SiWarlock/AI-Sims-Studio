...
You routed each Step-9 item hot during the session, per the **canonical matrix in `docs/orchestrator-briefing.md`** (loaded at `/orchestrate-start` — don't re-copy it here). Verify each landed: grep the lesson title in `<area>/LESSONS.md` + its linked index row in `<area>/CLAUDE.md`; `git diff` any `ARCHITECTURE.md` edits; grep Carry-forward / the phase for routed TODOs; confirm any deferment was escalated. The **most-likely-to-slip** is *Completed work → ticked checkbox* — Step 3.
...
- _(production-grade, optional)_ a report-only `<area>` audit one-liner (new-vs-baseline) if run this round — the blocking run stays at `/phase-exit`
