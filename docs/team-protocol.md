# Team Protocol — AI Sims Creator (Lead Playbook)
...
`apps/desktop/` (the desktop UI) · `services/pipeline/` (the Python pipeline sidecar) · `workers/export/` (the Sims DBPF export worker) · `workers/blender/` (the Blender mesh/GEOM worker, bpy) · `packages/contracts/` (the shared contracts package, pydantic→TS codegen) · `evals/` (the eval harnesses, LangSmith-native + metric layer)
...
Naming: **`<track>-<area>-<role>`** when parallel teams run (e.g. `frontend-team-orchestrator`), else `<area>-<role>` (e.g. `desktop-orchestrator`) — full rule in root `CLAUDE.md` "Naming + cross-bleed prevention." The `<track>` values come from the `IMPLEMENTATION_PLAN.md` Track map, not invented ad-hoc.
...
**The Parallelization plan's Track map drives worktrees proactively.** When the Phase/Track DAG has ≥2 parallel-eligible tracks, each track runs in its **own git worktree** (`git worktree add ../AISimsStudio-<track> track/<track>`, provisioned by that track's `/team-start <track>` Step 2.5) with its **own team** (lead + orch + per-area implementer). Single-working-tree is the fallback for a single-track (serial) plan, or a DAG that never branches. ("Explicit `git add <path>`, never `git add -A`" matters more than ever with parallel worktrees.)
