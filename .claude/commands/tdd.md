...
**Area-resolved commands.** `/tdd` runs from an implementer's area cwd; resolve the per-area test/typecheck/lint commands referenced below from your cwd basename:

```bash
case "$(basename "$PWD")" in
  desktop)   TEST_CMD="pnpm test:run";    TEST_ONE="pnpm test:run <path>";    TYPECHECK="pnpm typecheck"; LINT="pnpm lint" ;;
  pipeline)  TEST_CMD="uv run pytest";    TEST_ONE="uv run pytest <path> -v";  TYPECHECK="uv run mypy services/pipeline"; LINT="uv run ruff check ." ;;
  export)    TEST_CMD="pnpm test:run";    TEST_ONE="pnpm test:run <path>";    TYPECHECK="pnpm typecheck"; LINT="pnpm lint" ;;
  blender)   TEST_CMD="uv run pytest";    TEST_ONE="uv run pytest <path> -v";  TYPECHECK="uv run mypy workers/blender"; LINT="uv run ruff check ." ;;
  contracts) TEST_CMD="uv run pytest";    TEST_ONE="uv run pytest <path> -v";  TYPECHECK="uv run mypy packages/contracts"; LINT="uv run ruff check ." ;;
  evals)     TEST_CMD="uv run pytest";    TEST_ONE="uv run pytest <path> -v";  TYPECHECK="uv run mypy evals"; LINT="uv run ruff check ." ;;
  *) echo "Unknown area cwd: $PWD — run /tdd from a code-area directory"; exit 1 ;;
esac
```
